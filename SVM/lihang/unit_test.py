...
def test_e73():
    print("This ex is for introduce H and phi have not only one expression.")
    pass
...
def test_mlia():
    ...
    clf.fit(X, y)
    print("test_mlia: alpha is %s b is %s" % (str(clf.alpha[clf.alpha > 0.001]), str(clf.b)))
    print("test_mlia: support vector %s " % str(X[clf.alpha > 0.001, :]))
